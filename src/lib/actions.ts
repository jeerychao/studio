...
import {
  ...
  // Removed: generateSubnetsFromParent,
  // Removed: getPrefixFromRequiredHosts,
} from "./ip-utils";
...
export async function createSubnetAction(
  ...
  try {
    ...
    if (!newSubnetProperties) {
      throw new AppError('Failed to parse CIDR properties even after initial validation.', 500, 'CIDR_PARSE_UNEXPECTED_ERROR', '无法解析有效的 CIDR 属性，这通常表示一个内部错误。');
    }
    ...
    if (existingSubnetByCidr) {
      throw new ResourceError(`子网 ${canonicalCidrToStore} 已存在。`, 'SUBNET_ALREADY_EXISTS', `子网 ${canonicalCidrToStore} 已存在，无法重复创建。`, 'cidr');
    }
    ...
    for (const existingSub of allExistingSubnets) {
      ...
      if (existingSubProps && newSubnetProperties && doSubnetsOverlap(newSubnetProperties, existingSubProps)) {
        throw new ResourceError(`提供的子网 ${canonicalCidrToStore} 与现有子网 ${existingSub.cidr} 重叠。`, 'SUBNET_OVERLAP_ERROR', `提供的子网 ${canonicalCidrToStore} 与现有子网 ${existingSub.cidr} 重叠。请选择一个不冲突的范围。`, 'cidr');
      }
    }
...
export async function updateSubnetAction(id: string, data: UpdateSubnetData, performingUserId?: string): Promise<ActionResponse<AppSubnet>> {
  ...
  try {
    ...
    if (data.cidr && data.cidr !== subnetToUpdate.cidr) {
      validateCidrInput(data.cidr, 'cidr');
      const newSubnetProperties = getSubnetPropertiesFromCidr(data.cidr);
      if (!newSubnetProperties) { throw new AppError('Failed to parse new CIDR properties after validation.', 500, 'CIDR_PARSE_UNEXPECTED_ERROR', '无法解析新的有效 CIDR 属性。'); }
      newSubnetPropertiesForOverlapCheck = newSubnetProperties;
      const newCanonicalCidr = data.cidr; newCanonicalCidrForLog = newCanonicalCidr;
      const conflictingSubnetByCidr = await prisma.subnet.findFirst({ where: { cidr: newCanonicalCidr, NOT: { id } } });
      if (conflictingSubnetByCidr) { throw new ResourceError(`子网 ${newCanonicalCidr} 已存在。`, 'SUBNET_ALREADY_EXISTS', `新的 CIDR ${newCanonicalCidr} 与现有子网冲突。`, 'cidr'); }
      ...
      for (const existingSub of otherExistingSubnets) {
        ...
        if (existingSubProps && newSubnetPropertiesForOverlapCheck && doSubnetsOverlap(newSubnetPropertiesForOverlapCheck, existingSubProps)) {
          throw new ResourceError(`更新后的子网 ${newCanonicalCidr} 与现有子网 ${existingSub.cidr} 重叠。`, 'SUBNET_OVERLAP_ERROR', `更新后的子网 ${newCanonicalCidr} 与现有子网 ${existingSub.cidr} 重叠。请选择一个不冲突的范围。`, 'cidr');
        }
      }
      updateData.cidr = newCanonicalCidr; updateData.networkAddress = newSubnetProperties.networkAddress; updateData.subnetMask = newSubnetProperties.subnetMask; updateData.ipRange = newSubnetProperties.ipRange || null;
      ...
      for (const ip of allocatedIpsInSubnet) {
        if (!isIpInCidrRange(ip.ipAddress, newSubnetProperties)) {
          await prisma.iPAddress.update({ where: { id: ip.id }, data: { status: "free", allocatedTo: null, subnet: { disconnect: true }, vlan: { disconnect: true } } });
    ...
    if (Object.keys(updateData).length === 0) {
       const currentAppSubnet: AppSubnet = { ...subnetToUpdate, vlanId: subnetToUpdate.vlanId || undefined, description: subnetToUpdate.description || undefined, ipRange: subnetToUpdate.ipRange || undefined, utilization: await calculateSubnetUtilization(id) };
...
export async function deleteSubnetAction(id: string, performingUserId?: string): Promise<ActionResponse> {
  ...
  try {
    ...
    if (subnetToDelete.vlanId && subnetToDelete.vlanId.trim() !== "") { const vlanNumberForMessage = subnetToDelete.vlan ? `VLAN ${subnetToDelete.vlan.vlanNumber}` : `VLAN (ID: ${subnetToDelete.vlanId})`; throw new ResourceError(`子网 ${subnetToDelete.cidr} 已关联到 ${vlanNumberForMessage}。`, 'SUBNET_HAS_VLAN_ASSOCIATION', `无法删除子网 ${subnetToDelete.cidr}，因为它已关联到 ${vlanNumberForMessage}。请先解除其 VLAN 关联。`); }
    const allocatedIpsCount = await prisma.iPAddress.count({ where: { subnetId: id, status: 'allocated' } });
    if (allocatedIpsCount > 0) { throw new ResourceError(`子网 ${subnetToDelete.cidr} 中仍有 ${allocatedIpsCount} 个已分配的 IP 地址。`, 'SUBNET_HAS_ALLOCATED_IPS', `无法删除子网 ${subnetToDelete.cidr}，因为它仍包含已分配的 IP 地址。请先将这些 IP 释放或移至其他子网。`); }
    const reservedIpsCount = await prisma.iPAddress.count({ where: { subnetId: id, status: 'reserved' } });
    if (reservedIpsCount > 0) { throw new ResourceError(`子网 ${subnetToDelete.cidr} 中仍有 ${reservedIpsCount} 个预留状态的 IP 地址。`, 'SUBNET_HAS_RESERVED_IPS', `无法删除子网 ${subnetToDelete.cidr}，因为它仍包含预留状态的 IP 地址。请将这些 IP 状态改为空闲或移至其他子网。`); }
    const freeIpsInSubnet = await prisma.iPAddress.findMany({ where: { subnetId: id, status: 'free' } });
    for (const ip of freeIpsInSubnet) {
      await prisma.iPAddress.update({ where: { id: ip.id }, data: { subnet: { disconnect: true }, vlan: { disconnect: true } } });
      await prisma.auditLog.create({ data: { userId: auditUser.userId, username: auditUser.username, action: 'auto_disassociate_ip_on_subnet_delete', details: `IP ${ip.ipAddress} 已从子网 ${subnetToDelete.cidr} 解除关联（变为全局空闲），因为子网被删除。` } });
    }
...
export async function batchDeleteSubnetsAction(ids: string[], performingUserId?: string): Promise<BatchDeleteResult> {
  ...
  for (const id of ids) {
    try {
      ...
      if (subnetToDelete.vlanId && subnetToDelete.vlanId.trim() !== "") { const vlanNumberForMessage = subnetToDelete.vlan ? `VLAN ${subnetToDelete.vlan.vlanNumber}` : `VLAN (ID: ${subnetToDelete.vlanId})`; throw new ResourceError(`子网 ${subnetToDelete.cidr} 已关联到 ${vlanNumberForMessage}，无法删除。`, 'SUBNET_HAS_VLAN_ASSOCIATION_BATCH', `子网 ${subnetToDelete.cidr} 已关联到 ${vlanNumberForMessage}。请先解除其 VLAN 关联。`); }
      const allocatedIpsCount = await prisma.iPAddress.count({ where: { subnetId: id, status: 'allocated' } });
      if (allocatedIpsCount > 0) { throw new ResourceError(`子网 ${subnetToDelete.cidr} 中仍有 ${allocatedIpsCount} 个已分配的 IP 地址，无法删除。`, 'SUBNET_HAS_ALLOCATED_IPS_BATCH', `子网 ${subnetToDelete.cidr} 仍包含已分配的 IP 地址。`); }
      const reservedIpsCount = await prisma.iPAddress.count({ where: { subnetId: id, status: 'reserved' } });
      if (reservedIpsCount > 0) { throw new ResourceError(`子网 ${subnetToDelete.cidr} 中仍有 ${reservedIpsCount} 个预留的 IP 地址，无法删除。`, 'SUBNET_HAS_RESERVED_IPS_BATCH', `子网 ${subnetToDelete.cidr} 仍包含预留的 IP 地址。`); }
      const freeIpsInSubnet = await prisma.iPAddress.findMany({ where: { subnetId: id, status: 'free' } });
      for (const ip of freeIpsInSubnet) { await prisma.iPAddress.update({ where: { id: ip.id }, data: { subnet: { disconnect: true }, vlan: { disconnect: true } } }); }
      await prisma.subnet.delete({ where: { id } });
  ...
  if (deletedSubnetCidrs.length > 0) {
    await prisma.auditLog.create({ data: { userId: auditUser.userId, username: auditUser.username, action: 'batch_delete_subnet', details: `批量删除了 ${deletedSubnetCidrs.length} 个子网: ${deletedSubnetCidrs.join(', ')}。失败 ${failureDetails.length} 个。` } });
    revalidatePath("/subnets"); revalidatePath("/ip-addresses"); revalidatePath("/dashboard"); revalidatePath("/query");
  }
...
// Removed batchDivideAndCreateSubnetsAction and related types
...
export async function getVLANsAction(params?: FetchParams): Promise<PaginatedResponse<AppVLAN>> {
  ...
  const appVlans: AppVLAN[] = await Promise.all(vlansFromDb.map(async (vlan) => {
    ...
    const directIpCount = await prisma.iPAddress.count({ where: { vlanId: vlan.id } });
    return { ...vlan, name: vlan.name || undefined, description: vlan.description || undefined, subnetCount: subnetCount + directIpCount };
  }));
...
export async function createVLANAction(data: Omit<AppVLAN, "id" | "subnetCount">, performingUserId?: string): Promise<ActionResponse<AppVLAN>> {
  ...
  try {
    ...
    if (existingVLAN) { throw new ResourceError(`VLAN ${data.vlanNumber} 已存在。`, 'VLAN_EXISTS', `VLAN ${data.vlanNumber} 已存在，无法重复创建。`, 'vlanNumber'); }
...
export async function batchCreateVLANsAction(vlansToCreateInput: Array<{ vlanNumber: number; name?: string; description?: string; }>, performingUserId?: string): Promise<BatchVlanCreationResult> {
  ...
  const createdVlanSummaries: string[] = [];
  for (const vlanInput of vlansToCreateInput) {
    try {
      ...
      if (existingVLAN) { throw new ResourceError(`VLAN ${vlanInput.vlanNumber} 已存在。`, 'VLAN_EXISTS', undefined, 'startVlanNumber'); }
      ...
      createdVlanSummaries.push(`${newVlan.vlanNumber}${newVlan.name ? ` (${newVlan.name})` : ''}`);
      successCount++;
    } catch (e: unknown) {
  ...
  try {
    ...
    if (data.hasOwnProperty('vlanNumber') && data.vlanNumber !== undefined) {
      ...
      if (data.vlanNumber !== vlanToUpdate.vlanNumber) {
        const existingVLAN = await prisma.vLAN.findUnique({ where: { vlanNumber: data.vlanNumber } });
        if (existingVLAN && existingVLAN.id !== id) { throw new ResourceError(`已存在另一个 VLAN 号码为 ${data.vlanNumber} 的 VLAN。`, 'VLAN_EXISTS', `VLAN 号码 ${data.vlanNumber} 已被其他 VLAN 使用。`, 'vlanNumber'); }
      }
    ...
    if (Object.keys(updatePayload).length === 0) {
        ...
        const currentVLANApp: AppVLAN = { ...vlanToUpdate, name: vlanToUpdate.name || undefined, description: vlanToUpdate.description || undefined, subnetCount: subnetCount + directIpCount };
    ...
    const appVlan: AppVLAN = { ...updatedVLAN, name: updatedVLAN.name || undefined, description: updatedVLAN.description || undefined, subnetCount: subnetCount + directIpCount };
...
export async function deleteVLANAction(id: string, performingUserId?: string): Promise<ActionResponse> {
  ...
  try {
    ...
    if (subnetsUsingVlanCount > 0) { throw new ResourceError(`无法删除 VLAN ${vlanToDelete.vlanNumber} (${vlanToDelete.name || '无名称'})。它已分配给 ${subnetsUsingVlanCount} 个子网。请先解除关联。`, 'VLAN_IN_USE_SUBNET', `无法删除 VLAN ${vlanToDelete.vlanNumber} (${vlanToDelete.name || '无名称'})，因为它仍被 ${subnetsUsingVlanCount} 个子网使用。`); }
    const ipsUsingVlanDirectlyCount = await prisma.iPAddress.count({ where: { vlanId: id } });
    if (ipsUsingVlanDirectlyCount > 0) { throw new ResourceError(`无法删除 VLAN ${vlanToDelete.vlanNumber} (${vlanToDelete.name || '无名称'})。它已直接分配给 ${ipsUsingVlanDirectlyCount} 个 IP 地址。请先移除直接分配。`, 'VLAN_IN_USE_IP', `无法删除 VLAN ${vlanToDelete.vlanNumber} (${vlanToDelete.name || '无名称'})，因为它仍被 ${ipsUsingVlanDirectlyCount} 个 IP 地址直接使用。`); }
...
export async function batchDeleteVLANsAction(ids: string[], performingUserId?: string): Promise<BatchDeleteResult> {
  ...
  for (const id of ids) {
    try {
      ...
      const subnetsUsingVlanCount = await prisma.subnet.count({ where: { vlanId: id } });
      if (subnetsUsingVlanCount > 0) { throw new ResourceError(`VLAN ${vlanToDelete.vlanNumber} (${vlanToDelete.name || '无名称'}) 已分配给 ${subnetsUsingVlanCount} 个子网。`, 'VLAN_IN_USE_SUBNET_BATCH', `VLAN ${vlanToDelete.vlanNumber} (${vlanToDelete.name || '无名称'}) 仍被 ${subnetsUsingVlanCount} 个子网使用。`); }
      const ipsUsingVlanDirectlyCount = await prisma.iPAddress.count({ where: { vlanId: id } });
      if (ipsUsingVlanDirectlyCount > 0) { throw new ResourceError(`VLAN ${vlanToDelete.vlanNumber} (${vlanToDelete.name || '无名称'}) 已直接分配给 ${ipsUsingVlanDirectlyCount} 个 IP 地址。`, 'VLAN_IN_USE_IP_BATCH', `VLAN ${vlanToDelete.vlanNumber} (${vlanToDelete.name || '无名称'}) 仍被 ${ipsUsingVlanDirectlyCount} 个 IP 地址直接使用。`); }
      await prisma.vLAN.delete({ where: { id } });
      deletedVlanSummaries.push(`${vlanToDelete.vlanNumber}${vlanToDelete.name ? ` (${vlanToDelete.name})` : ''}`);
      successCount++;
    } catch (error: unknown) {
      const errorResponse = createActionErrorResponse(error, `${actionName}_single`);
      const vlanInfo = await prisma.vLAN.findUnique({ where: { id } });
      const itemIdentifier = vlanInfo ? `VLAN ${vlanInfo.vlanNumber} (${vlanInfo.name || '无名称'})` : `ID ${id}`;
      failureDetails.push({ id, itemIdentifier, error: errorResponse.userMessage });
    }
  }
  if (deletedVlanSummaries.length > 0) {
    await prisma.auditLog.create({ data: { userId: auditUser.userId, username: auditUser.username, action: 'batch_delete_vlan', details: `批量删除了 ${deletedVlanSummaries.length} 个 VLAN: ${deletedVlanSummaries.join(', ')}。失败 ${failureDetails.length} 个。` } });
    revalidatePath("/vlans"); revalidatePath("/subnets"); revalidatePath("/ip-addresses"); revalidatePath("/query");
  }
...
export async function getIPAddressesAction(params?: FetchParams): Promise<PaginatedResponse<AppIPAddressWithRelations>> {
  ...
  const includeClause = { subnet: { include: { vlan: { select: { vlanNumber: true, name: true } } } }, vlan: { select: { vlanNumber: true, name: true } } };
  ...
  if (params?.subnetId) {
    ...
    if (params?.page && params?.pageSize) {
        ...
        paginatedDbItems = await prisma.iPAddress.findMany({ where: whereClause, include: includeClause, orderBy: orderByClause, skip: skip, take: pageSize }) as PrismaIPAddressWithRelations[];
    } else {
        const allIPs = await prisma.iPAddress.findMany({ where: whereClause, include: includeClause, orderBy: orderByClause }) as PrismaIPAddressWithRelations[];
  ...
  const appIps: AppIPAddressWithRelations[] = paginatedDbItems.map(ip => ({
    id: ip.id, ipAddress: ip.ipAddress, status: ip.status as AppIPAddressStatusType,
    allocatedTo: ip.allocatedTo || undefined, description: ip.description || undefined, lastSeen: ip.lastSeen?.toISOString() || undefined,
    subnetId: ip.subnetId || undefined, vlanId: ip.vlanId || undefined,
    subnet: ip.subnet ? { id: ip.subnet.id, cidr: ip.subnet.cidr, networkAddress: ip.subnet.networkAddress, vlan: ip.subnet.vlan ? { vlanNumber: ip.subnet.vlan.vlanNumber, name: ip.subnet.vlan.name || undefined } : null } : null,
    vlan: ip.vlan ? { vlanNumber: ip.vlan.vlanNumber, name: ip.vlan.name || undefined } : null,
  }));
...
export async function createIPAddressAction(data: Omit<AppIPAddress, "id">, performingUserId?: string): Promise<ActionResponse<AppIPAddress>> {
    ...
    try {
        ...
        if (ipParts.some(part => isNaN(part) || part < 0 || part > 255) || ipParts.length !== 4) { throw new ValidationError(`无效的 IP 地址格式: ${data.ipAddress}`, 'ipAddress', data.ipAddress, `IP 地址 ${data.ipAddress} 格式无效。`); }
        if (!data.subnetId && (prismaStatus === 'allocated' || prismaStatus === 'reserved')) { throw new ValidationError("对于“已分配”或“预留”状态的 IP，除非设置为空闲，否则必须选择一个子网。", 'subnetId'); }
        if (data.subnetId) {
            ...
            if (!parsedTargetSubnetCidr) { throw new AppError(`目标子网 ${targetSubnet.cidr} 的 CIDR 无效，无法验证 IP。`, 500, 'SUBNET_CIDR_INVALID_FOR_IP_CHECK'); }
            if (!isIpInCidrRange(data.ipAddress, parsedTargetSubnetCidr)) { throw new ValidationError(`IP ${data.ipAddress} 不在子网 ${targetSubnet.cidr} (${parsedTargetSubnetCidr.networkAddress} - ${parsedTargetSubnetCidr.broadcastAddress}) 的范围内。`, 'ipAddress', data.ipAddress, `IP 地址 ${data.ipAddress} 不在所选子网的有效范围内。`); }
            const existingIPInSubnet = await prisma.iPAddress.findFirst({ where: { ipAddress: data.ipAddress, subnetId: data.subnetId } });
            if (existingIPInSubnet) { throw new ResourceError(`IP ${data.ipAddress} 已存在于子网 ${targetSubnet.networkAddress} 中。`, 'IP_EXISTS_IN_SUBNET', `IP 地址 ${data.ipAddress} 已存在于所选子网中。`, 'ipAddress'); }
        } else {
            const globallyExistingIP = await prisma.iPAddress.findFirst({ where: { ipAddress: data.ipAddress, subnetId: null } });
            if (globallyExistingIP) { throw new ResourceError(`IP ${data.ipAddress} 已存在于全局池中 (未分配给任何子网)。`, 'IP_EXISTS_GLOBALLY', `IP 地址 ${data.ipAddress} 已存在于全局池中。`, 'ipAddress'); }
        }
        const createPayload: Prisma.IPAddressCreateInput = { ipAddress: data.ipAddress, status: prismaStatus, allocatedTo: data.allocatedTo || null, description: data.description || null, lastSeen: data.lastSeen ? new Date(data.lastSeen) : null };
        if (data.subnetId) { createPayload.subnet = { connect: { id: data.subnetId } }; }
        if (data.vlanId === null) {} else if (data.vlanId && data.vlanId.trim() !== "") {
            const vlanExists = await prisma.vLAN.findUnique({ where: { id: data.vlanId }});
            if (!vlanExists) { throw new NotFoundError(`VLAN ID: ${data.vlanId}`, `为 IP 地址选择的 VLAN 不存在。`, 'vlanId'); }
            createPayload.vlan = { connect: { id: data.vlanId } };
        }
...
export async function batchCreateIPAddressesAction(payload: { startIp: string; endIp: string; subnetId: string; vlanId?: string | null; description?: string; status: AppIPAddressStatusType; }, performingUserId?: string): Promise<BatchIpCreationResult> {
  const auditUser = await getAuditUserInfo(performingUserId);
  let successCount = 0;
  ...
  try {
    ...
    if (vlanId && vlanId.trim() !== "") { const vlanExists = await prisma.vLAN.findUnique({ where: { id: vlanId } }); if (!vlanExists) { throw new NotFoundError(`VLAN ID: ${vlanId}`, "为批量 IP 创建选择的 VLAN 不存在。", 'vlanId'); } }
    ...
    for (; currentIpNum <= endIpNum; currentIpNum++) {
      ...
      try {
        ...
        const existingIPInSubnet = await prisma.iPAddress.findFirst({ where: { ipAddress: currentIpStr, subnetId: subnetId } });
        if (existingIPInSubnet) { throw new ResourceError(`IP ${currentIpStr} 已存在于子网 ${targetSubnet.networkAddress} 中。`, 'IP_EXISTS_IN_SUBNET', undefined, 'startIp/endIp'); }
        const createPayload: Prisma.IPAddressCreateInput = { ipAddress: currentIpStr, status: status, allocatedTo: status === 'allocated' ? (description || '批量分配') : null, description: description || null };
        if (subnetId) { createPayload.subnet = { connect: { id: subnetId } }; }
        if (vlanId && vlanId.trim() !== "") { createPayload.vlan = { connect: { id: vlanId } }; }
  ...
  } catch (e: unknown) { const errorResponse = createActionErrorResponse(e, 'batchCreateIPAddressesAction_setup'); return { successCount: 0, failureDetails: [{ ipAttempted: `${startIp}-${endIp}`, error: errorResponse.userMessage }] }; }
...
export interface UpdateIPAddressData { ipAddress?: string; subnetId?: string | undefined; vlanId?: string | null | undefined; status?: AppIPAddressStatusType; allocatedTo?: string | null | undefined; description?: string | null | undefined; lastSeen?: string | null | undefined; }
export async function updateIPAddressAction(id: string, data: UpdateIPAddressData, performingUserId?: string): Promise<ActionResponse<AppIPAddress>> {
  ...
  try {
    ...
    if (data.hasOwnProperty('ipAddress') && data.ipAddress !== undefined && data.ipAddress !== ipToUpdate.ipAddress) {
      const ipParts = data.ipAddress.split('.').map(Number);
      if (ipParts.some(part => isNaN(part) || part < 0 || part > 255) || ipParts.length !== 4) { throw new ValidationError(`无效的 IP 地址格式更新: ${data.ipAddress}`, 'ipAddress', data.ipAddress); }
      updateData.ipAddress = data.ipAddress; finalIpAddress = data.ipAddress;
    }
    ...
    if (data.hasOwnProperty('vlanId')) { const vlanIdToSet = data.vlanId; if (vlanIdToSet === null) { updateData.vlan = { disconnect: true }; } else if (vlanIdToSet) { if (!(await prisma.vLAN.findUnique({where: {id: vlanIdToSet}}))) { throw new NotFoundError(`VLAN ID: ${vlanIdToSet}`, `为 IP 地址选择的 VLAN 不存在。`, 'vlanId'); } updateData.vlan = { connect: { id: vlanIdToSet } }; } }
    ...
    if (data.hasOwnProperty('subnetId')) {
        if (newSubnetId) {
            ...
            if (!isIpInCidrRange(finalIpAddress, parsedTargetSubnetCidr)) { throw new ValidationError(`IP ${finalIpAddress} 不在子网 ${targetSubnet.cidr} 的范围内。`, 'ipAddress/subnetId', finalIpAddress); }
            if (finalIpAddress !== ipToUpdate.ipAddress || newSubnetId !== ipToUpdate.subnetId) { const conflictingIP = await prisma.iPAddress.findFirst({ where: { ipAddress: finalIpAddress, subnetId: newSubnetId, NOT: { id } } }); if (conflictingIP) throw new ResourceError(`IP ${finalIpAddress} 已存在于子网 ${targetSubnet.networkAddress} 中。`, 'IP_EXISTS_IN_SUBNET', undefined, 'ipAddress'); }
            ...
            if (finalIpAddress !== ipToUpdate.ipAddress || ipToUpdate.subnetId !== null) { const globallyConflictingIP = await prisma.iPAddress.findFirst({ where: { ipAddress: finalIpAddress, subnetId: null, NOT: { id } } }); if (globallyConflictingIP) throw new ResourceError(`IP ${finalIpAddress} 已存在于全局池中。`, 'IP_EXISTS_GLOBALLY', undefined, 'ipAddress'); }
        ...
        if (!isIpInCidrRange(finalIpAddress, parsedCurrentSubnetCidr)) { throw new ValidationError(`新 IP ${finalIpAddress} 不在当前子网 ${currentSubnet.cidr} 的范围内。`, 'ipAddress', finalIpAddress); }
...
export async function deleteIPAddressAction(id: string, performingUserId?: string): Promise<ActionResponse> {
  ...
  try {
    const auditUser = await getAuditUserInfo(performingUserId);
    const ipToDelete = await prisma.iPAddress.findUnique({ where: { id }, include: { subnet: { select: { cidr: true, vlanId: true, vlan: { select: { vlanNumber: true } } } }, vlan: { select: { vlanNumber: true } } } });
    if (!ipToDelete) { throw new NotFoundError(`IP 地址 ID: ${id}`, `要删除的 IP 地址未找到。`); }
    if (ipToDelete.status === 'allocated' || ipToDelete.status === 'reserved') { throw new ResourceError(`IP 地址 ${ipToDelete.ipAddress} 状态为 "${ipToDelete.status}"。`, 'IP_ADDRESS_IN_USE_STATUS', `无法删除 IP 地址 ${ipToDelete.ipAddress}，因为其状态为 "${ipToDelete.status === 'allocated' ? '已分配' : '预留'}"。请先将其状态更改为空闲。`); }
    const hasDirectVlan = ipToDelete.vlanId && ipToDelete.vlanId.trim() !== "";
    if (hasDirectVlan) { const directVlanNumber = ipToDelete.vlan?.vlanNumber || ipToDelete.vlanId; throw new ResourceError(`IP 地址 ${ipToDelete.ipAddress} 直接关联到 VLAN ${directVlanNumber}。`, 'IP_ADDRESS_HAS_DIRECT_VLAN', `无法删除 IP 地址 ${ipToDelete.ipAddress}，因为它直接关联到 VLAN ${directVlanNumber}。请先移除其 VLAN 关联 (在 IP 地址编辑中设为“从子网继承或无”)。`); }
...
export async function batchDeleteIPAddressesAction(ids: string[], performingUserId?: string): Promise<BatchDeleteResult> {
  ...
  for (const id of ids) {
    try {
      const ipToDelete = await prisma.iPAddress.findUnique({ where: { id }, include: { vlan: { select: { vlanNumber: true } } } });
      ...
      if (ipToDelete.vlanId && ipToDelete.vlanId.trim() !== "") { const directVlanNumber = ipToDelete.vlan?.vlanNumber || ipToDelete.vlanId; throw new ResourceError(`IP 地址 ${ipToDelete.ipAddress} 直接关联到 VLAN ${directVlanNumber}。`, 'IP_ADDRESS_HAS_DIRECT_VLAN_BATCH', `IP ${ipToDelete.ipAddress} 直接关联到 VLAN ${directVlanNumber}。`); }
      await prisma.iPAddress.delete({ where: { id } });
  ...
  if (deletedIpAddresses.length > 0) {
    await prisma.auditLog.create({ data: { userId: auditUser.userId, username: auditUser.username, action: 'batch_delete_ip_address', details: `批量删除了 ${deletedIpAddresses.length} 个 IP 地址: ${deletedIpAddresses.join(', ')}。失败 ${failureDetails.length} 个。` } });
    revalidatePath("/ip-addresses"); revalidatePath("/dashboard"); revalidatePath("/subnets"); revalidatePath("/query");
  }
...
export async function createUserAction(data: Omit<AppUser, "id" | "lastLogin" | "roleName"> & { password: string, avatar?: string }, performingUserId?: string): Promise<ActionResponse<FetchedUserDetails>> {
  ...
  try {
    const auditUser = await getAuditUserInfo(performingUserId);
    if (await prisma.user.findUnique({ where: { email: data.email } })) { throw new ResourceError(`提供的邮箱 '${data.email}' 已被其他用户使用。`, 'USER_EMAIL_EXISTS', `邮箱 '${data.email}' 已被注册。`, 'email'); }
    if (await prisma.user.findUnique({ where: { username: data.username } })) { throw new ResourceError(`提供的用户名 '${data.username}' 已被其他用户使用。`, 'USER_USERNAME_EXISTS', `用户名 '${data.username}' 已被注册。`, 'username'); }
...
export async function updateUserAction(id: string, data: Partial<Omit<AppUser, "id" | "roleName">> & { password?: string }, performingUserId?: string): Promise<ActionResponse<FetchedUserDetails>> {
  ...
  try {
    const auditUser = await getAuditUserInfo(performingUserId);
    const userToUpdate = await prisma.user.findUnique({ where: { id }, include: { role: true } });
    if (!userToUpdate) { throw new NotFoundError(`用户 ID: ${id}`, `要更新的用户未找到。`); }
    if (!userToUpdate.role || !userToUpdate.role.name) { throw new AppError(`用户 ${id} 没有关联的角色或角色名无效。`, 500, 'USER_MISSING_ROLE_ON_UPDATE', `用户 ${userToUpdate.username} 的角色信息丢失，无法更新。`); }
    const updateData: Prisma.UserUpdateInput = {};
    if (data.hasOwnProperty('username') && data.username !== undefined && data.username !== userToUpdate.username) { if (await prisma.user.findFirst({ where: { username: data.username, NOT: { id } } })) { throw new ResourceError(`提供的用户名 '${data.username}' 已被其他用户使用。`, 'USER_USERNAME_EXISTS', `用户名 '${data.username}' 已被占用。`, 'username'); } updateData.username = data.username; }
    if (data.hasOwnProperty('email') && data.email !== undefined && data.email !== userToUpdate.email) { if (await prisma.user.findFirst({ where: { email: data.email, NOT: { id } } })) { throw new ResourceError(`提供的邮箱 '${data.email}' 已被其他用户使用。`, 'USER_EMAIL_EXISTS', `邮箱 '${data.email}' 已被占用。`, 'email'); } updateData.email = data.email; }
    if (data.hasOwnProperty('roleId') && data.roleId !== undefined && data.roleId !== userToUpdate.roleId) {
      ...
      if (!newRole || !newRole.name) { throw new NotFoundError(`角色 ID: ${data.roleId}`, `选择的角色不存在或无效。`, 'roleId'); }
      if (userToUpdate.role.name === 'Administrator' && newRole.name !== 'Administrator') { const adminCount = await prisma.user.count({ where: { role: { name: 'Administrator' } } }); if (adminCount <= 1) { throw new ResourceError('无法更改最后一位管理员的角色。', 'LAST_ADMIN_ROLE_CHANGE_NOT_ALLOWED', '系统中必须至少保留一名管理员。无法更改最后一名管理员的角色。', 'roleId'); } }
...
export async function updateOwnPasswordAction(userId: string, payload: UpdateOwnPasswordPayload): Promise<ActionResponse<{ message: string }>> {
  ...
  try {
    ...
    if (!user) { throw new NotFoundError(`用户 ID: ${userId}`, "执行操作的用户未找到。"); }
    if (!payload.currentPassword) { throw new ValidationError("当前密码是必需的。", "currentPassword"); }
    if (user.password !== payload.currentPassword) { throw new AuthError("当前密码不匹配。", "当前密码不正确。", "currentPassword"); }
...
export async function deleteUserAction(id: string, performingUserId?: string): Promise<ActionResponse> {
  ...
  try {
    ...
    if (!userToDelete.role || !userToDelete.role.name) { throw new AppError("无法删除用户：关联的角色信息无效。", 500, 'USER_ROLE_INVALID_PRE_DELETE', "无法删除用户，因为其角色信息已损坏。"); }
    if (performingUserId && id === performingUserId) { throw new ResourceError("无法删除当前登录的用户。", "DELETE_SELF_NOT_ALLOWED", "您不能删除自己的账户。"); }
    if (userToDelete.role.name === "Administrator") { const adminCount = await prisma.user.count({ where: { role: { name: "Administrator" } } }); if (adminCount <= 1) { throw new ResourceError("无法删除最后一个管理员用户。", "LAST_ADMIN_DELETE_NOT_ALLOWED", "系统中必须至少保留一个管理员用户。"); } }
    await prisma.auditLog.updateMany({ where: { userId: id }, data: { userId: null, username: `已删除用户 (${userToDelete.username})` } });
    await prisma.user.delete({ where: { id } });
    await prisma.auditLog.create({ data: { userId: auditUser.userId, username: auditUser.username, action: 'delete_user', details: `删除了用户 ${userToDelete.username} (ID: ${id})。` } });
    revalidatePath("/users"); revalidatePath("/roles");
...
export async function updateRoleAction(id: string, data: Partial<Omit<AppRole, "id" | "userCount" | "name">> & { permissions?: AppPermissionIdType[] }, performingUserId?: string): Promise<ActionResponse<AppRole>> {
  ...
  try {
    ...
    if (!roleToUpdate || !roleToUpdate.name) { throw new NotFoundError(`角色 ID: ${id}`, `要更新的角色未找到或名称无效。`); }
    if (roleToUpdate.id === ADMIN_ROLE_ID && data.permissions) { throw new ResourceError('无法修改内置 "Administrator" 角色的权限。', 'ADMIN_ROLE_PERMISSIONS_FIXED', '系统内置的 "Administrator" 角色的权限集是固定的，无法修改。您可以修改其描述。', 'permissions'); }
    ...
    if (data.permissions && roleToUpdate.id !== ADMIN_ROLE_ID) {
      ...
      if (!allProvidedPermissionsAreValid) { throw new ValidationError('提供的权限列表中包含无效的权限ID。', 'permissions'); }
...
export async function deleteAuditLogAction(id: string, performingUserId?: string): Promise<ActionResponse> {
  ...
  try {
    ...
    if (!logToDelete) { throw new NotFoundError(`审计日志 ID: ${id}`, `要删除的审计日志条目未找到。`); }
    await prisma.auditLog.delete({ where: { id } });
    try { await prisma.auditLog.create({ data: { userId: auditUser.userId, username: auditUser.username, action: 'delete_audit_log_entry', details: `删除了审计日志条目 ID ${id} (原操作: ${logToDelete.action}, 原用户: ${logToDelete.username || 'N/A'}, 原时间戳: ${logToDelete.timestamp.toISOString()})` } }); } catch (logError) { logger.error('Failed to create audit log for audit log deletion', logError as Error, { originalLogId: id }, actionName); }
...
export async function batchDeleteAuditLogsAction(ids: string[], performingUserId?: string): Promise<BatchDeleteResult> {
  ...
  for (const id of ids) {
    try {
      const logToDelete = await prisma.auditLog.findUnique({ where: { id } });
      if (!logToDelete) { failureDetails.push({ id, itemIdentifier: `ID ${id}`, error: '审计日志条目未找到。' }); continue; }
      await prisma.auditLog.delete({ where: { id } });
  ...
  if (deletedLogSummaries.length > 0) { try { await prisma.auditLog.create({ data: { userId: auditUser.userId, username: auditUser.username, action: 'batch_delete_audit_log', details: `批量删除了 ${deletedLogSummaries.length} 个审计日志条目。失败 ${failureDetails.length} 个。` } }); } catch (logError) { logger.error('Failed to create audit log for batch audit log deletion', logError as Error, { deletedCount: deletedLogSummaries.length }, actionName); } revalidatePath("/audit-logs"); }
  ...
  try {
    ...
    const results: SubnetQueryResult[] = await Promise.all(
      subnetsFromDb.map(async (subnet) => {
        ...
        return { id: subnet.id, cidr: subnet.cidr, description: subnet.description || undefined, vlanNumber: subnet.vlan?.vlanNumber, vlanName: subnet.vlan?.name || undefined, totalUsableIPs, allocatedIPsCount, dbFreeIPsCount, reservedIPsCount };
      })
    );
    return { success: true, data: { data: results, totalCount, currentPage: page, totalPages, pageSize } };
  } catch (error: unknown) {
    logger.error(`Error in ${actionName}`, error as Error, { inputData: params }, actionName);
    return { success: false, error: createActionErrorResponse(error, actionName) };
  }
}
...
export async function queryVlansAction(params: QueryToolParams): Promise<ActionResponse<PaginatedResponse<VlanQueryResult>>> {
  ...
  try {
    ...
    const vlansFromDb = await prisma.vLAN.findMany({
      where: whereClause,
      include: { subnets: { select: { id: true, cidr: true, description: true }, take: 10 }, ipAddresses: { select: { id: true, ipAddress: true, description: true }, take: 10 }, _count: { select: { subnets: true, ipAddresses: true } } },
      orderBy: { vlanNumber: 'asc' }, skip, take: pageSize,
    });
    const results: VlanQueryResult[] = vlansFromDb.map(vlan => ({
      id: vlan.id, vlanNumber: vlan.vlanNumber, name: vlan.name || undefined, description: vlan.description || undefined,
...
export async function getSubnetFreeIpDetailsAction(subnetId: string): Promise<ActionResponse<SubnetFreeIpDetails>> {
  ...
  try {
    const subnet = await prisma.subnet.findUnique({ where: { id: subnetId } });
    if (!subnet) { throw new NotFoundError(`子网 ID: ${subnetId}`, '子网未找到。'); }
    const subnetProperties = getSubnetPropertiesFromCidr(subnet.cidr);
    if (!subnetProperties) { throw new AppError(`无法解析子网 ${subnet.cidr} 的属性。`, 500, 'SUBNET_PROP_PARSE_ERROR'); }
    const totalUsableIPs = getUsableIpCount(subnetProperties.prefix);
    const dbIpsInSubnet = await prisma.iPAddress.findMany({ where: { subnetId: subnet.id }, select: { ipAddress: true, status: true } });
    const dbAllocatedIpsSet = new Set<number>(); const dbReservedIpsSet = new Set<number>();
    dbIpsInSubnet.forEach(ip => { const ipNum = ipToNumber(ip.ipAddress); if (ip.status === 'allocated') { dbAllocatedIpsSet.add(ipNum); } else if (ip.status === 'reserved') { dbReservedIpsSet.add(ipNum); } });
    const dbAllocatedIPsCount = dbAllocatedIpsSet.size; const dbReservedIPsCount = dbReservedIpsSet.size;
    const calculatedAvailableNumericIps: number[] = [];
    if (subnetProperties.firstUsableIp && subnetProperties.lastUsableIp) {
      const firstUsableNum = ipToNumber(subnetProperties.firstUsableIp); const lastUsableNum = ipToNumber(subnetProperties.lastUsableIp);
      for (let currentIpNum = firstUsableNum; currentIpNum <= lastUsableNum; currentIpNum++) { if (!dbAllocatedIpsSet.has(currentIpNum) && !dbReservedIpsSet.has(currentIpNum)) { calculatedAvailableNumericIps.push(currentIpNum); } }
    } else if (subnetProperties.prefix === 32) { const singleIpNum = ipToNumber(subnetProperties.networkAddress); if (!dbAllocatedIpsSet.has(singleIpNum) && !dbReservedIpsSet.has(singleIpNum)) { calculatedAvailableNumericIps.push(singleIpNum); }
    } else if (subnetProperties.prefix === 31) { const firstIpNum = ipToNumber(subnetProperties.networkAddress); const secondIpNum = firstIpNum + 1; if (!dbAllocatedIpsSet.has(firstIpNum) && !dbReservedIpsSet.has(firstIpNum)) { calculatedAvailableNumericIps.push(firstIpNum); } if (!dbAllocatedIpsSet.has(secondIpNum) && !dbReservedIpsSet.has(secondIpNum)) { calculatedAvailableNumericIps.push(secondIpNum); } }
    ...
    const result: SubnetFreeIpDetails = { subnetId: subnet.id, subnetCidr: subnet.cidr, totalUsableIPs, dbAllocatedIPsCount, dbReservedIPsCount, calculatedAvailableIPsCount, calculatedAvailableIpRanges };
