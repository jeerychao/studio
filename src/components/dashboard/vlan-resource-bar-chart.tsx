...
export function VlanResourceBarChart({ data }: VlanResourceBarChartProps) {
  if (!data || data.length === 0) {
    return <div className="text-center text-muted-foreground p-4 h-full flex items-center justify-center">无 VLAN 资源数据可显示。</div>;
  }

  return (
    <ChartContainer config={{}} className="w-full h-full min-h-[200px]">
      <BarChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 60 }}> {/* Increased bottom margin */}
        <CartesianGrid strokeDasharray="3 3" vertical={false}/>
        <XAxis dataKey="name" tick={{ fontSize: 10 }} interval={0} angle={-45} textAnchor="end" height={70} /> {/* Increased height for angled labels */}
