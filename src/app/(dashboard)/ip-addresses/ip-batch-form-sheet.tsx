      ...
      <SheetContent className="sm:max-w-lg w-full flex flex-col p-0">
        ...
        <Form {...form}><form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col flex-grow overflow-hidden">
            <ScrollArea className="flex-1 px-6 pt-4"><div className="space-y-4 pb-4">
                ...
                <FormField control={form.control} name="commonSelectedAccessType" render={({ field }) => (<FormItem><FormLabel>接入方式</FormLabel><Select onValueChange={field.onChange} value={field.value || NO_SELECTION_SENTINEL}><FormControl><SelectTrigger><SelectValue placeholder="选择接入方式" /></SelectTrigger></FormControl><SelectContent><SelectItem value={NO_SELECTION_SENTINEL}>-- 无 --</SelectItem><SelectItem value="汇聚">汇聚</SelectItem><SelectItem value="独立专线">独立专线</SelectItem></SelectContent></Select><FormMessage /></FormItem>)} />
