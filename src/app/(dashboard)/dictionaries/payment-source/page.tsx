...
import { CreditCard, Loader2, PlusCircle, Edit, Trash2, ShieldAlert } from "lucide-react"; // Using CreditCard as placeholder
import { useCurrentUser, hasPermission } from "@/hooks/use-current-user";
import { PERMISSIONS, type PaymentSourceDictionary, type PaginatedResponse } from "@/types";
...
function PaymentSourceDictionaryView() {
  const [dictData, setDictData] = React.useState<PaginatedResponse<PaymentSourceDictionary> | null>(null);
  ...
  const fetchData = React.useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      if (hasPermission(currentUser, PERMISSIONS.VIEW_DICTIONARY_PAYMENT_SOURCE)) {
        const fetchedResult = await getPaymentSourceDictionariesAction({ page: currentPage, pageSize: ITEMS_PER_PAGE });
         if (fetchedResult.success && fetchedResult.data) {
          ...
          toast({ title: "获取数据错误", description: fetchedResult.error?.userMessage || "未能加载付费来源字典数据。", variant: "destructive" });
 ...
 const handleChangeSuccess = React.useCallback(async () => {
    ...
    const paginationInfo = await getPaymentSourceDictionariesAction(queryParams);
  ...
  if (!currentUser || !hasPermission(currentUser, PERMISSIONS.VIEW_DICTIONARY_PAYMENT_SOURCE)) {
    return (
  ...
  const canCreate = hasPermission(currentUser, PERMISSIONS.CREATE_DICTIONARY_PAYMENT_SOURCE);
  const canEdit = hasPermission(currentUser, PERMISSIONS.EDIT_DICTIONARY_PAYMENT_SOURCE);
  const canDelete = hasPermission(currentUser, PERMISSIONS.DELETE_DICTIONARY_PAYMENT_SOURCE);

  const pageActionButtons = (
    <div className="flex flex-col sm:flex-row gap-2">
      {canDelete && selectedIds.size > 0 && <BatchDeleteConfirmationDialog selectedIds={selectedIds} itemTypeDisplayName="付费来源字典条目" batchDeleteAction={batchDeletePaymentSourceDictionariesAction} onBatchDeleted={fetchData} />}
      {canCreate && <PaymentSourceDictionaryFormSheet onDataChange={handleChangeSuccess} buttonProps={{className: "w-full sm:w-auto"}}/>}
    </div>
  );
  ...
  return (
    <>
      ...
      <Card>
        <CardHeader><CardTitle>付费来源列表</CardTitle><CardDescription>显示 {itemsToDisplay.length} 条，共 {finalTotalCount} 条付费来源字典条目。</CardDescription></CardHeader>
        <CardContent>
          {dataIsAvailable ? (
            <>
              <Table>
                    ...
                    <TableRow key={item.id} data-state={selectedIds.has(item.id) ? "selected" : ""}>
                      ...
                      {(canEdit || canDelete) && (
                        <TableCell className="text-right">
                          {canEdit && <PaymentSourceDictionaryFormSheet dictionaryEntry={item} onDataChange={fetchData}><Button variant="ghost" size="icon" aria-label="编辑条目"><Edit className="h-4 w-4" /></Button></PaymentSourceDictionaryFormSheet>}
                          {canDelete && <DeleteConfirmationDialog itemId={item.id} itemName={item.sourceName} deleteAction={deletePaymentSourceDictionaryAction} onDeleted={fetchData} triggerButton={<Button variant="ghost" size="icon" aria-label="删除条目"><Trash2 className="h-4 w-4" /></Button>} />}
                        </TableCell>
                      )}
                    </TableRow>
                  ))}
             ...
             <div className="text-center py-10">
              ...
              {canCreate && <PaymentSourceDictionaryFormSheet onDataChange={handleChangeSuccess} buttonProps={{className: "mt-4"}}/>}
            </div>
          )}
        </CardContent>
      </Card>
