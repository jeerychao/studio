...
// Top-level check to see if the script execution even starts
console.log('--- PRISMA SEED SCRIPT: Initializing script execution (top of file) ---');

// Wrap ALL imports and the rest of the script in a try-catch to see if an import itself fails.
try {
  const dotenv = require('dotenv');
  const path = require('path');
  
  console.log('--- PRISMA SEED SCRIPT: Attempting dotenv.config() ---');
  try {
    const envPath = path.resolve(__dirname, '../.env');
    const result = dotenv.config({ path: envPath });
    if (result.error) {
      console.warn(`--- PRISMA SEED SCRIPT: dotenv.config() error loading from ${envPath}:`, result.error.message);
    } else if (Object.keys(result.parsed || {}).length > 0) {
      console.log(`--- PRISMA SEED SCRIPT: dotenv.config() loaded ${Object.keys(result.parsed || {}).length} variables successfully from ${envPath}. ENCRYPTION_KEY from .env: ${process.env.ENCRYPTION_KEY ? process.env.ENCRYPTION_KEY.substring(0,5)+'...' : 'NOT SET IN .env'}`);
    } else {
      console.log(`--- PRISMA SEED SCRIPT: dotenv.config() ran, but no variables parsed from ${envPath}. File might be empty or only comments.`);
    }
  } catch (dotenvError: any) {
    console.error("--- PRISMA SEED SCRIPT: CRITICAL ERROR during dotenv.config() call ---", dotenvError.message, dotenvError.stack);
  }
  console.log(`--- PRISMA SEED SCRIPT: After dotenv.config(). Current ENCRYPTION_KEY in process.env: ${process.env.ENCRYPTION_KEY ? process.env.ENCRYPTION_KEY.substring(0,5)+'...' : 'NOT SET'}`);

  // Now proceed with other imports, which might depend on process.env being set up
  const { PrismaClient, Prisma } = require('@prisma/client'); // Using require for this debug phase
  console.log('--- PRISMA SEED SCRIPT: PrismaClient imported ---');
  
  const dataModule = require('../src/lib/data');
  console.log('--- PRISMA SEED SCRIPT: ../src/lib/data imported ---');
  
  const cryptoUtilsModule = require('../src/lib/crypto-utils');
  console.log('--- PRISMA SEED SCRIPT: ../src/lib/crypto-utils imported ---');


  const {
    mockPermissions: seedPermissionsData,
    mockRoles: seedRolesData,
    mockVLANs: seedVLANsData,
    mockSubnets: seedSubnetsData,
    seedIPsData,
    mockAuditLogs: seedAuditLogsData,
    ADMIN_ROLE_ID: SEED_ADMIN_ROLE_ID,
    OPERATOR_ROLE_ID: SEED_OPERATOR_ROLE_ID,
    VIEWER_ROLE_ID: SEED_VIEWER_ROLE_ID,
    mockDeviceDictionaries, // This was mockLocalDeviceDictionaries
    mockPaymentSourceDictionaries,
    mockAccessTypeDictionaries,
    mockInterfaceTypeDictionaries, // This was mockNetworkInterfaceTypeDictionaries
  } = dataModule;

  const { encrypt } = cryptoUtilsModule;
  // const typeUser = require('../src/types'); // type User as AppUser - types not needed at runtime for require

  console.log("--- PRISMA SEED SCRIPT: All imports seem to be resolved. Proceeding to PrismaClient instantiation. ---");

  const prisma = new PrismaClient();
  console.log("--- PRISMA SEED SCRIPT: PrismaClient instantiated. ---");


  (async () => {
    try {
      console.log("--- PRISMA SEED SCRIPT: Async IIFE started. ATTEMPTING MAIN FUNCTION EXECUTION ---");
      await main(); // main function defined below
      console.log("--- PRISMA SEED SCRIPT: MAIN FUNCTION EXECUTION FINISHED (SUCCESSFULLY OR WITH CAUGHT ERROR INSIDE MAIN) ---");
    } catch (e: any) {
      console.error('--- PRISMA SEED SCRIPT: UNCAUGHT ERROR IN TOP-LEVEL ASYNC IIFE ---');
      console.error("Full error object:", e);
      if (e && typeof e === 'object') {
        if ('name' in e) console.error("Error name:", e.name);
        if ('message' in e) console.error("Error message:", e.message);
        if ('stack' in e) console.error("Error stack:", e.stack);
        if ('code' in e) console.error("Error code (if any):", e.code);
      }
      process.exit(1);
    } finally {
      console.log("--- PRISMA SEED SCRIPT: TOP-LEVEL ASYNC IIFE FINALLY BLOCK ---");
  ...
  async function main() {
    ...
    try {
      ...
      console.log(`[Seed Script MAIN] Attempting to use ENCRYPTION_KEY from process.env (should be set by .env or Prisma CLI): ${process.env.ENCRYPTION_KEY ? process.env.ENCRYPTION_KEY.substring(0, 5) + '...' : 'NOT SET IN PROCESS.ENV'}`);
      ...
      for (const p of seedPermissionsData) {
        await prisma.permission.upsert({
          ...
          create: { id: p.id as string, name: p.name, group: p.group, description: p.description },
        });
      }
      ...
      for (const roleData of seedRolesData) {
        ...
        const conflictingRoleByName = await prisma.role.findFirst({ where: { name: roleData.name, NOT: { id: roleData.id } } });
      ...
      for (const roleData of seedRolesData) {
        ...
        if (roleData.id === SEED_ADMIN_ROLE_ID) {
          ...
          const validRolePermissions = roleData.permissions.filter(appPermId => allDbPermissions.some(dbPerm => dbPerm.id === (appPermId as string)));
          permissionsToSet = validRolePermissions.map(appPermId => ({ id: appPermId as string }));
        }
        await prisma.role.update({ where: { id: roleData.id }, data: { permissions: { set: permissionsToSet } } });
      }
      ...
      for (const userData of initialUsersToSeedPlain) {
        ...
        await prisma.user.upsert({
          ...
          update: { id: userData.id, username: userData.username, password: encryptedPassword, phone: plainPhone, roleId: userData.roleId, avatar: userData.avatarPath, lastLogin: new Date() },
          create: { id: userData.id, username: userData.username, email: userData.email, password: encryptedPassword, phone: plainPhone, roleId: userData.roleId, avatar: userData.avatarPath, lastLogin: new Date() },
        });
      }
      ...
      for (const subnetData of seedSubnetsData) {
        ...
        await prisma.subnet.upsert({
          ...
          update: { cidr: subnetData.cidr, networkAddress: subnetData.networkAddress, subnetMask: subnetData.subnetMask, ipRange: subnetData.ipRange, name: subnetData.name, dhcpEnabled: subnetData.dhcpEnabled, description: subnetData.description, vlanId: subnetData.vlanId, },
          create: { id: subnetData.id, cidr: subnetData.cidr, networkAddress: subnetData.networkAddress, subnetMask: subnetData.subnetMask, ipRange: subnetData.ipRange, name: subnetData.name, dhcpEnabled: subnetData.dhcpEnabled, description: subnetData.description, vlanId: subnetData.vlanId, },
        });
      }
      ...
      for (const ipData of seedIPsData) {
        ...
        await prisma.iPAddress.upsert({
          where: { id: ipData.id },
          update: {
            ipAddress: ipData.ipAddress, status: ipData.status as string, isGateway: ipData.isGateway, allocatedTo: ipData.allocatedTo, usageUnit: ipData.usageUnit, contactPerson: ipData.contactPerson, phone: phone, description: ipData.description, subnetId: ipData.subnetId, directVlanId: ipData.directVlanId,
            peerUnitName: ipData.peerUnitName || null, peerDeviceName: ipData.peerDeviceName || null, peerPortName: ipData.peerPortName || null,
            selectedAccessType: ipData.selectedAccessType || null, selectedLocalDeviceName: ipData.selectedLocalDeviceName || null, selectedDevicePort: ipData.selectedDevicePort || null, selectedPaymentSource: ipData.selectedPaymentSource || null,
          },
          create: {
            id: ipData.id, ipAddress: ipData.ipAddress, status: ipData.status as string, isGateway: ipData.isGateway, allocatedTo: ipData.allocatedTo, usageUnit: ipData.usageUnit, contactPerson: ipData.contactPerson, phone: phone, description: ipData.description, subnetId: ipData.subnetId, directVlanId: ipData.directVlanId,
            peerUnitName: ipData.peerUnitName || null, peerDeviceName: ipData.peerDeviceName || null, peerPortName: ipData.peerPortName || null,
            selectedAccessType: ipData.selectedAccessType || null, selectedLocalDeviceName: ipData.selectedLocalDeviceName || null, selectedDevicePort: ipData.selectedDevicePort || null, selectedPaymentSource: ipData.selectedPaymentSource || null,
          },
        });
      ...
      for (const ddData of mockDeviceDictionaries) { // Renamed variable
        await prisma.deviceDictionary.upsert({ // Renamed model
      ...
      for (const itData of mockInterfaceTypeDictionaries) { // Renamed variable
        await prisma.interfaceTypeDictionary.upsert({ // Renamed model
      ...
      for (const logData of seedAuditLogsData) {
        ...
        if (!existingLog) {
          await prisma.auditLog.create({ data: { id: logData.id, userId: validUserId, username: validUsername, action: logData.action, details: logData.details, timestamp: logData.timestamp ? new Date(logData.timestamp) : new Date(), } });
        } else {
          await prisma.auditLog.update({ where: {id: logData.id}, data: { userId: validUserId, username: validUsername, action: logData.action, details: logData.details, timestamp: logData.timestamp ? new Date(logData.timestamp) : new Date(), } });
        }
      }
      ...
      console.error("Full error object in main:", e);
      if (e && typeof e === 'object') {
        if ('name' in e) console.error("Error name in main:", e.name);
        if ('message' in e) console.error("Error message in main:", e.message);
        if ('stack' in e) console.error("Error stack in main:", e.stack);
        if ('code' in e) console.error("Error code (if any):", e.code);
      }
      throw e;
    } finally {
  ...
  // This top-level catch is for import/require failures or other synchronous errors before the IIFE
  console.error('--- PRISMA SEED SCRIPT: FATAL ERROR DURING SCRIPT INITIALIZATION (IMPORTS/SETUP) ---');
  console.error("Full error object:", e);
  if (e && typeof e === 'object') {
    if ('name' in e) console.error("Error name:", e.name);
    if ('message' in e) console.error("Error message:", e.message);
